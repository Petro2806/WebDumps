...
MONTH_EXTRACTOR = re.compile(r"CC-MAIN-(\d{6})")
...
def s3_to_http(url: str) -> str:
    # s3://commoncrawl/crawl-data/...  ->  https://data.commoncrawl.org/crawl-data/...
    assert url.startswith("s3://commoncrawl/")
    return "https://data.commoncrawl.org/" + url[len("s3://commoncrawl/"):]

def http_stream_to_temp(url: str) -> str:
    s = requests.Session()
...
def open_file(path, mode="rb", profile=None):
    ...
    if path.startswith("s3://"):
        if path.startswith("s3://commoncrawl/"):
            http_url = s3_to_http(path)
            print(f"reading via HTTPS: {http_url}")
            local_path = http_stream_to_temp(http_url)
            return FileStream(local_path, "rb")
        if profile is None:
            s3 = boto3.resource("s3", config=Config(signature_version=UNSIGNED, max_pool_connections=32))
        else:
...
def open_file_old(path, mode="rb", profile=None):
    print("attempting to open file", path)
    if path.startswith("s3://"):
...
if __name__ == "__main__":
    ...
    if args.json_file_path.endswith(".json") or args.json_file_path.endswith(".json.gz"):
